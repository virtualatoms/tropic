...
import { Monomer, Reaction, Product, Parameters, Thermo, Metadata } from "@/lib/types";
...
export function MonomerInfoCard({ data }: { data: Monomer }) {
  const rows = [
    ...
    <InfoRow
      ...
      value={data.molecular_weight ? `${data.molecular_weight.toFixed(2)} g/mol` : null}
    />,
...
export function ProductInfoCard({ data }: { data: Product }) {
  const rows = [
    <InfoRow key="smiles" label="SMILES" value={data.smiles} />,
    <InfoRow key="repeating_units" label="Repeating Units" value={data.repeating_units} />,
    <InfoRow key="deg_of_poly" label="Degree of Polymerization" value={data.deg_of_poly} />,
    <InfoRow key="dispersity" label="Dispersity (Đ)" value={data.dispersity} />,
    <InfoRow key="n_avg_molar_mass" label="Number-Avg Molar Mass" value={data.n_avg_molar_mass} />,
    <InfoRow key="m_avg_molar_mass" label="Mass-Avg Molar Mass" value={data.m_avg_molar_mass} />,
  ];

  return (
    <Card withBorder shadow="sm" radius="md">
      <CardSection withBorder inheritPadding py="xs">
        <Center><Text fw={700}>Product Information</Text></Center>
      </CardSection>
      <Table mt="xs"><Table.Tbody>{rows}</Table.Tbody></Table>
    </Card>
  );
}
...
export function ParametersInfoCard({ data }: { data: Parameters }) {
    const rows = [
      <InfoRow key="is_experimental" label="Is Experimental" value={data.is_experimental ? "Yes" : "No"} />,
      <InfoRow key="temperature" label="Temperature (K)" value={data.temperature} />,
      <InfoRow key="pressure" label="Pressure (atm)" value={data.pressure} />,
      <InfoRow key="state_summary" label="State (Monomer-Polymer)" value={data.state_summary} />,
      <InfoRow key="initiator_smiles" label="Initiator SMILES" value={data.initiator_smiles} />,
      ...
      <InfoRow key="method_calc" label="Calculation Method" value={data.method_calc} />,
  ...
  return (
    <Card withBorder shadow="sm" radius="md" mt={25}>
      <CardSection withBorder inheritPadding py="xs">
        <Center><Text fw={700}>Parameters</Text></Center>
      </CardSection>
      <Table mt="xs"><Table.Tbody>{rows}</Table.Tbody></Table>
    </Card>
  );
...
export function ThermoInfoCard({ data }: { data: Thermo }) {
  ...
  const rows = [
    <InfoRow
      key="delta_h"
      label="ΔH"
      value={formatValueWithUncertainty(data.delta_h, data.delta_h_std, "kJ/mol")}
    />,
    <InfoRow
      key="delta_s"
      label="ΔS"
      value={formatValueWithUncertainty(data.delta_s, data.delta_s_std, "J/mol·K")}
    />,
    <InfoRow
      key="delta_g"
      label="ΔG"
  ...
  return (
    <Card withBorder shadow="sm" radius="md" mt={25}>
      <CardSection withBorder inheritPadding py="xs">
        <Center><Text fw={700}>Thermodynamics</Text></Center>
      </CardSection>
      <Table mt="xs"><Table.Tbody>{rows}</Table.Tbody></Table>
    </Card>
  );
...
export function MetadataInfoCard({ data }: { data: Metadata }) {
  ...
  return (
    <Card withBorder shadow="sm" radius="md" mt={25}>
      <CardSection withBorder inheritPadding py="xs">
        <Center><Text fw={700}>Metadata</Text></Center>
      </CardSection>
      <Table mt="xs"><Table.Tbody>{rows}</Table.Tbody></Table>
    </Card>
  );
