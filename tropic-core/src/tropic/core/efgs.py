...
def get_fgs(mol):
    ...
    for atom in mol.GetAtoms():
        if (atom.GetAtomicNum() not in (6, 1)) and (atom.GetIsAromatic() is True):
            ...
            for nbr in atom.GetNeighbors():
                if (
                    nbr.GetIsAromatic() == False
                ):  # Don't merge adjacent aromatic atoms into the same FG!!
                    for group in groups:
...
def get_carbonyl_envs(mol, functional_groups):
    ...
    pats_idx = mol.GetSubstructMatches(
        carbonyl_pattern
    )  # tupla of sets of matched atoms
    carbonyl_envs = []
    for group in functional_groups:
        ...
        for pat_idx in pats_idx:
            neigh_list = []
            if (
                len(set(pat_idx) & group) > 0
            ):  # identify the carbonyl corresponding in the group to that SMARTS match
                c_atom = mol.GetAtomWithIdx(pat_idx[0])  # Get the C of the carbonyl
                neigh_list = [
                    (
                        (pat_idx[0], x.GetIdx()),
                        mol.GetBondBetweenAtoms(pat_idx[0], x.GetIdx()),
                    )
                    for x in c_atom.GetNeighbors()
                    if ((x.GetIdx() not in group) and (x.GetAtomicNum() == 6))
                ]
            if len(neigh_list) > 0:
...
def get_freeval_envs(mol, functional_groups):
            ...
            for m in matches:
                ...
                if (pattern == oh_pattern) or (
                    ...
                    atom = mol.GetAtomWithIdx(
                        m[0]
                    )  # Retrieve O, N, or S of the hydroxyl, amine, thiol
                    for n_atom in atom.GetNeighbors():
                        if (
                            n_atom.GetAtomicNum() == 1
                        ):  # Will only preserve if explicit H, if not it remains "silent"
    ...
    for group in functional_groups:
        ...
        for idx in group:
            atom = mol.GetAtomWithIdx(idx)
            if (atom.GetAtomicNum() not in [6, 1]) and (
                atom.GetIsAromatic() == False
            ):  # heteroatom of the group
                neigh_list = [
                    (
                        (idx, x.GetIdx()),
                        mol.GetBondBetweenAtoms(idx, x.GetIdx()),
                        x.GetAtomicNum(),
                    )
                    for x in atom.GetNeighbors()
                    if (
                        (x.GetIdx() not in group)
                        and (x.GetIdx() not in preserve_hs)
                        and (x.GetAtomicNum() in (6, 1))
                        and (
                            mol.GetBondBetweenAtoms(idx, x.GetIdx()).GetIsAromatic()
                            == False
                        )
                    )
                ]
...
def get_singleno_envs(mol, functional_groups):
    ...
    for group in functional_groups:
        ...
        if (
            (len(group) == 1)
            and (mol.GetAtomWithIdx(list(group)[0]).GetAtomicNum() in [7, 8])
            and not (mol.GetAtomWithIdx(list(group)[0]).GetIsAromatic())
        ):
            ...
            if (
                ...
                for neigh_atom in atom.GetNeighbors():
                    if (neigh_atom.GetAtomicNum() == 6) and (
                        mol.GetBondBetweenAtoms(
                            atom.GetIdx(), neigh_atom.GetIdx()
                        ).GetBondType()
                        == Chem.rdchem.BondType.SINGLE
                    ):
                        type_carbon = (
                            "Car" if neigh_atom.GetIsAromatic() == True else "Cal"
                        )
                        singleno_env.add(
                            (
                                (list(group)[0], neigh_atom.GetIdx()),
                                mol.GetBondBetweenAtoms(
                                    list(group)[0], neigh_atom.GetIdx()
                                ),
                                type_carbon,
...
def get_dbond2car(mol, functional_groups):
    ...
    matches = tuple(
        [
            x
            for x in matches
            if mol.GetBondBetweenAtoms(x[0], x[1]).GetIsAromatic() == False
        ]
    )
    for group in functional_groups:
        db2car_env = set()
...
def col_mol(mol, functional_groups, rad=0.5, lw=2, width=300, height=250):
    ...
    for fg in functional_groups:
        ...
        els = "".join(
            sorted(list(set([mol.GetAtomWithIdx(idx).GetSymbol() for idx in fg])))
        )
...
def get_dec_fgs(mol, verbose=False):
        ...
        if len(fve) > 0:
            ...
            for j in range(len(fve)):
                ...
                if fv[2] == 1:
                    ...
                    fve[j] = (
                        (d_pair),
                        mol.GetBondBetweenAtoms(d_pair[0], d_pair[1]),
                        0,
                    )
    ...
    for i, group in enumerate(fgs):
        neigh_atoms = sorted(
            list(
                set(
                    [x[0][1] for x in cnes[i]]
                    + [x[0][1] for x in fves[i]]
                    + [x[0][1] for x in noes[i]]
                )
            )
        )
        cne = cnes[i]
        ...
        psmi_labs = [(list(x)[0][1], list(x)[2]) for x in noe if len(noe) > 0] + [
            (x, "Car") for x in db2car if len(db2car) > 0
        ]
        ...
        if len(neigh_atoms) > 0:  # There is decoration applicable
            ...
            for idx in R_idxs:
                ...
                r_group_atom.SetAtomMapNum(
                    int(label[-2])
                )  # Set the map number to match the R group number
            ...
            pseudo_smiles = Chem.MolToSmiles(
                fragment_with_r_groups, canonical=True, isomericSmiles=False
            )
            ...
            for atom in fragment_with_r_groups.GetAtoms():
                ...
                map_idx = list(amap.keys())[
                    list(amap.values()).index(idx)
                ]  # idx of atom in original molecule
                if (
                    map_idx in neigh_atoms
                ):  # Check if that position is an environment atom in original molecule
                    if map_idx in [x[0][1] for x in list(noe)]:
                        type_carbon = [x[2] for x in list(noe) if x[0][1] == map_idx][0]
                        fragment_with_r_groups.GetAtomWithIdx(idx).SetProp(
                            "atomLabel", type_carbon
                        )
                    else:
                        fragment_with_r_groups.GetAtomWithIdx(idx).SetProp(
                            "atomLabel", "R"
            ...
            if len(group) == 1:  # Single-atommed FG
                ...
                if atom.GetIsAromatic():
                    if atom.GetAtomicNum() == 7:
                        ...
                        fragment.GetAtomWithIdx(0).SetProp(
                            "atomLabel", "Nar" + charge_str
                        )
                    if atom.GetAtomicNum() == 8:
                        pseudo_smiles = "o" + charge_str
                        fragment.GetAtomWithIdx(0).SetProp(
                            "atomLabel", "Oar" + charge_str
                        )
                    if atom.GetAtomicNum() == 16:
                        pseudo_smiles = "s" + charge_str
                        fragment.GetAtomWithIdx(0).SetProp(
                            "atomLabel", "Sar" + charge_str
                        )
                    if atom.GetAtomicNum() == 15:
                        pseudo_smiles = "p" + charge_str
                        fragment.GetAtomWithIdx(0).SetProp(
                            "atomLabel", "Par" + charge_str
                        )
                    if atom.GetAtomicNum() == 34:
                        pseudo_smiles = "se" + charge_str
                        fragment.GetAtomWithIdx(0).SetProp(
                            "atomLabel", "Sear" + charge_str
                        )
                    if atom.GetAtomicNum() == 52:
                        pseudo_smiles = "te" + charge_str
                        fragment.GetAtomWithIdx(0).SetProp(
                            "atomLabel", "Tear" + charge_str
                        )
                else:
                    if atom.GetAtomicNum() == 7:
                        pseudo_smiles = "N" + charge_str
                        fragment.GetAtomWithIdx(0).SetProp(
                            "atomLabel", "N" + charge_str
                        )
                    if atom.GetAtomicNum() == 8:
                        pseudo_smiles = "O" + charge_str
                        fragment.GetAtomWithIdx(0).SetProp(
                            "atomLabel", "O" + charge_str
                        )
                    if atom.GetAtomicNum() == 16:
                        pseudo_smiles = "S" + charge_str
                        fragment.GetAtomWithIdx(0).SetProp(
                            "atomLabel", "S" + charge_str
                        )
                    if atom.GetAtomicNum() == 15:
                        pseudo_smiles = "P" + charge_str + "]"
                        fragment.GetAtomWithIdx(0).SetProp(
                            "atomLabel", "P" + charge_str
                        )
                    if atom.GetAtomicNum() == 34:
                        pseudo_smiles = "Se" + charge_str + "]"
                        fragment.GetAtomWithIdx(0).SetProp(
                            "atomLabel", "Se" + charge_str
                        )
                    if atom.GetAtomicNum() == 52:
                        pseudo_smiles = "Te" + charge_str + "]"
                        fragment.GetAtomWithIdx(0).SetProp(
                            "atomLabel", "Te" + charge_str
                        )
                ...
                for atom in fragment.GetAtoms():
                    ...
                    charge = atom.GetFormalCharge()
                    charge_str = (
                        "\u207a" if charge == 1 else "\u207b" if charge == -1 else ""
                    )
                    if atom.GetIsAromatic():
                        if atom.GetAtomicNum() == 6:
                            fragment.GetAtomWithIdx(idx).SetProp(
                                "atomLabel", "Car" + charge_str
                            )
                        if atom.GetAtomicNum() == 7:
                            fragment.GetAtomWithIdx(idx).SetProp(
                                "atomLabel", "Nar" + charge_str
        ...
        pseudo_smiles = pseudo_smiles.replace("[s-]", "[Sar-]")
        pseudo_smiles = re.sub(
            r"(?<!A)(s\+)", "[Sar+]", pseudo_smiles
        )  # To avoid issues with As
        pseudo_smiles = re.sub(
            r"(?<!A)(s-)", "[Sar-]", pseudo_smiles
        )  # To avoid issues with As
        pseudo_smiles = re.sub(
